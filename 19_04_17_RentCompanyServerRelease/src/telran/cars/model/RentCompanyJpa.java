...
public class RentCompanyJpa extends AbstractRentCompany {
	...
	public List<Car> getDriverCars(long licenseId) {
		List<Car> cars = recordRepository.findByDriverLicenseId(licenseId).stream().map(r -> getCarDto(r.getCar())).distinct()
	...
	public List<Driver> getCarDrivers(String regNumber) {
		return recordRepository.findByCarRegNumber(regNumber).stream().map(r -> getDriverDto(r.getDriver())).distinct()
				.collect(Collectors.toList());
	}
	...
	public List<RentRecord> getRentRecordsAtDates(LocalDate from, LocalDate to) {
		return recordRepository.findByRentDateBetween(from, to).stream().map(this::getRentRecordDto)
				.collect(Collectors.toList());
	}
	...
	private List<RentRecord> actualRemove(CarJpa carJpa) {
		List<RecordJpa> recordsForRemove = recordRepository.findByCarRegNumber(carJpa.getRegNumber()).stream().collect(Collectors.toList());
